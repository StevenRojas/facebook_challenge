...
from app.utils.utils import parse_predict_arguments, get_flowers_names
from app.network.predictor import Predictor
...
def check_arguments(args):
    if not os.path.exists(args.image_path):
        sys.exit("[ERROR] Image not found: {}".format(args.image_path))
...
def load_checkpoint(predictor, args):
    ...
    if result is False:
        sys.exit("[ERROR] Error while loading checkpoint: {}".format(predictor.last_error))
    else:
...
def load_model(predictor, device):
    ...
    if result is False:
        sys.exit("[ERROR] Error while loading model: {}".format(predictor.last_error))
    else:
